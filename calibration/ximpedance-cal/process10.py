...
KNOWN_CURRENT_TO_VOLTAGE_RATIOS: tuple[tuple[float, ...], tuple[float, ...]] = (
    (-100.0e-6, -90.0e-6, -80.0e-6, -70.0e-6, -60.0e-6, -50.0e-6, -40.0e-6, -30.0e-6,
     -25.0e-6, -20.0e-6, -15.0e-6, -10.0e-6),
    (1139.12e-3, 1040.13e-3, 940.406e-3, 837.0e-3, 740.009e-3, 640.073e-3, 541.527e-3,
     435.797e-3, 387.163e-3, 346.562e-3, 310.329e-3, 234.27e-3),
)
...
def demarcate_sweep(sample_points: np.ndarray) -> list[list[tuple[float, float]]]:
    ...
    # transients.
    alpha = 2.0  # The coefficient for determining demarcation windows.
    ...
    gamma = int(4)  # The number of samples to slice away from the head and tail of each
                    # demarcation.
    delta = 0.0005   # The minimum stddev for marking a demarcation
